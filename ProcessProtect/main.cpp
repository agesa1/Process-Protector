...
extern PVOID g_registration_handle;
...
extern "C" VOID DriverUnload(PDRIVER_OBJECT DriverObject) {
	...
	if (g_registration_handle) {
		ObUnRegisterCallbacks(g_registration_handle);
	}

	// Unhide and free protected process list
	while (g_protected_processes != nullptr) {
		auto entry = CONTAINING_RECORD(g_protected_processes, ProtectProcessEntry, next);
		
		// Unhide before freeing
		if (entry->is_hidden) {
			unhide_process(entry->args.pid);
		}
		
		g_protected_processes = g_protected_processes->Next;
		ExFreePoolWithTag(entry, 0x1337);
	}
...
extern "C" NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {
	...
	nt_status = register_protectors();
	if (!NT_SUCCESS(nt_status)) {
		DbgPrint("Couldn't register callbacks\n");
		IoDeleteSymbolicLink(&win32_name);
		IoDeleteDevice(device_object);
		return nt_status;
	}
